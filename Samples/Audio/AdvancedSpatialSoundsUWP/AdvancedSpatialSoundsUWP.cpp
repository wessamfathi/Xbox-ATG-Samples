...
VOID CALLBACK SpatialWorkCallback(_Inout_ PTP_CALLBACK_INSTANCE Instance, _Inout_opt_ PVOID Context, _Inout_ PTP_WORK Work)
{
	...
	{
		while (Sink->m_playingSound)
		{
			// Wait for a signal from the audio-engine to start the next processing pass 
			if (Sink->m_renderer->m_bufferCompletionEvent)
			{
				if (WaitForSingleObject(Sink->m_renderer->m_bufferCompletionEvent, INFINITE) != WAIT_OBJECT_0)
				{
					continue;
				}
			}
            ...
            for (std::vector<PointSound>::iterator it = Sink->m_pointSounds.begin(); it != Sink->m_pointSounds.end(); it++)
            {
                ...
                if (!it->isPlaying)
                {
                    ...
                    //Workaround - if this isn't here, the objects never are returned to the resource pool
                    Sleep(5);
...
void Sample::Update(DX::StepTimer const& timer)
{
    ...
    {
        if (m_threadActive && m_playingSound)
        {
...
void Sample::Render()
{
	...
	pos.y += 30;
    swprintf_s(str, L"Use Spacebar to pause/unpause playback and UP/DOWN to add/remove a sound");
    m_font->DrawString(m_spriteBatch.get(), str, pos, ATG::Colors::White);
    pos.y += 60;
